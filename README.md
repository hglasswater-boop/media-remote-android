...
6. Android 17では、アプリの「ローカルネットワーク」アクセスを許可する
7. 長時間後にCast接続できない場合は、アプリまたは常駐通知の「Cast待受を再起動」を押す（再生中のYouTube Musicは止まりません）
...
Favorite Songsなどのプレイリスト名が、操作側で「再生キュー」に変わる未解決の互換性問題があります。純正Chromecastでは元の名前が表示されることが報告されています。曲と位置が正しくても、プレイリスト情報の同期が完了したとは扱いません。
